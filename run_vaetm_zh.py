...
def main():
    ...
    parser.add_option('-k', dest='n_topics', default=100,
    ...
    train_prefix = 'train'
    train_X, vocab, train_labels, label_names, label_type, col_sel ,num= load_data(input_dir, train_prefix, label_file_name, vocab_size=vocab_size)
    ...
    if test_prefix=='test':
        test_X, _, test_labels, _, _, _ ,_= load_data(input_dir, test_prefix, label_file_name, vocab=vocab)
    ...
    model = train(model, network_architecture, train_X, train_labels, vocab, regularize=auto_regularize, training_epochs=n_epochs, batch_size=batch_size, rng=rng, bn_anneal=bn_anneal, X_dev=dev_X)
    ...
    if test_X is not None:
        perplexity = evaluate_perplexity(model, test_X, test_labels, eta_bn_prop=0.0)
    ...
    if n_labels > 0:
        ...
        predict_labels_and_evaluate(model, train_X, train_labels, None, output_dir, subset='train')

        if dev_X is not None:
            predict_labels_and_evaluate(model, dev_X, dev_labels, None, output_dir, subset='dev')

        if test_X is not None:
            predict_labels_and_evaluate(model, test_X, test_labels, None, output_dir, subset='test')
...
def load_data(input_dir:str, input_prefix:str, label_file_name=None, vocab_size=None, vocab=None, col_sel=None):
    print("Loading data")
    ...
    return X, vocab, labels, label_names, label_type, col_sel,num
...
def create_minibatch(X, Y, batch_size=200, rng=None):
    """
    ...
    while True:
        ...
        if Y is not None:
            yield X[ixs, :].astype('float32'), Y[ixs, :].astype('float32')
        else:
            yield X[ixs, :].astype('float32'), None
...
def train(model, network_architecture, X, Y, vocab, batch_size=200, training_epochs=100, display_step=5, min_weights_sq=1e-7, regularize=False, bn_anneal=True, init_eta_bn_prop=1.0, rng=None, X_dev=None):

    n_train, dv = X.shape
    mb_gen = create_minibatch(X, Y, batch_size=batch_size, rng=rng)
    ...
    for epoch in range(training_epochs):
        ...
        for i in range(total_batch):
            # get a minibatch
            batch_xs, batch_ys = next(mb_gen)
            # do one update, passing in the data, regularization strengths, and bn
            loss, cls_loss, pred = model.fit(batch_xs, batch_ys, l2_strengths=l2_strengths, eta_bn_prop=eta_bn_prop, kld_weight=kld_weight)
...
def evaluate_perplexity(model, X, Y, eta_bn_prop=1.0, n_samples=0):
    """
    ...
    losses = model.get_losses(X, Y, eta_bn_prop=eta_bn_prop, n_samples=n_samples)
