...
class VaeTm(object):
    ...
    def __init__(self, network_architecture, alpha=1.0,
                 learning_rate=0.001, batch_size=100, init_embeddings=None, 
        ...
        if entity_embeddings is not None:
            self.sess.run(self.network_weights['entity_embeddings'].assign(entity_embeddings))
    ...
    def _create_network(self):
        ...
        print('shape of en0_x:', en0_x.shape)
        encoder_parts.append(en0_x)

        # en0_entity = tf.matmul(self.x, self.network_weights['entity_embeddings'])
        # print('shape of en0_entity:', en0_entity.shape)
        # encoder_parts.append(en0_entity)

        # add label if we have them
        if n_labels > 0:
            print('label_emb_dim:',label_emb_dim)
            if label_emb_dim > 0:
    ...
    def _initialize_weights(self):
        ...
        all_weights['embeddings'] = tf.Variable(xavier_init(dv, embedding_dim), trainable=False)
        all_weights['entity_embeddings'] = tf.Variable(xavier_init(dv, ENTITY_DIM), trainable=False)
    ...
    def fit(self, X, Y, l2_strengths, eta_bn_prop=1.0, kld_weight=1.0, keep_prob=0.8):
        """
        ...
        if Y is not None:
            opt, loss, classifier_loss, pred = self.sess.run((self.optimizer, self.loss, self.classifier_loss, self.pred_y), feed_dict={self.x: X, self.y: Y, self.keep_prob: .8, self.l2_strengths: l2_strengths, self.eta_bn_prop: eta_bn_prop, self.kld_weight: kld_weight, self.theta_input: theta_input})
        else:
            opt, loss = self.sess.run((self.optimizer, self.loss), feed_dict={self.x: X, self.keep_prob: keep_prob, self.l2_strengths: l2_strengths, self.eta_bn_prop: eta_bn_prop, self.kld_weight: kld_weight, self.theta_input: theta_input})
            classifier_loss = 0
            pred = -1
    ...
    def predict(self, X, C, eta_bn_prop=0.0):
        ...
        if self.network_architecture['n_labels']>0:
            theta, pred = self.sess.run((self.theta, self.y_recon), feed_dict={self.x: X, self.y: Y, self.keep_prob: 1.0, self.l2_strengths: l2_strengths, self.batch_size: 1, self.var_scale: 0.0, self.is_training: False, self.theta_input: theta_input, self.eta_bn_prop: eta_bn_prop})
        else:
            theta = self.sess.run((self.theta, ), feed_dict={self.x: X, self.keep_prob: 1.0, self.l2_strengths: l2_strengths, self.batch_size: 1, self.var_scale: 0.0, self.is_training: False, self.theta_input: theta_input, self.eta_bn_prop: eta_bn_prop})
            pred = -1
        return theta, pred
    ...
    def get_losses(self, X, Y, eta_bn_prop=0.0, n_samples=0):
        """
        ...
        theta_input = np.zeros([batch_size, self.network_architecture['n_topics']]).astype('float32')
        if n_samples == 0:
            losses = self.sess.run(self.losses, feed_dict={self.x: X, self.keep_prob: 1.0, self.l2_strengths: l2_strengths, self.batch_size: batch_size, self.var_scale: 0.0, self.is_training: False, self.theta_input: theta_input, self.eta_bn_prop: eta_bn_prop})
        else:
            losses = self.sess.run(self.losses, feed_dict={self.x: X, self.keep_prob: 1.0, self.l2_strengths: l2_strengths, self.batch_size: batch_size, self.var_scale: 1.0, self.is_training: False, self.theta_input: theta_input, self.eta_bn_prop: eta_bn_prop})
    ...
    def compute_theta(self, X, Y):
        ...
        theta = self.sess.run(self.theta, feed_dict={self.x: X, self.keep_prob: 1.0, self.l2_strengths: l2_strengths, self.var_scale: 0.0, self.batch_size: batch_size, self.is_training: False, self.theta_input: theta_input})
        return theta
