...
ENTITY_CATEGORY = 'WikiData.KB.100d.zh'
DIM = 100
...
def list_wiki_labels(start_id:int, size:int)->List[WikiLabel]:
    results = []
    query = ("SELECT id, label, title_cn FROM label_title "
             "WHERE id BETWEEN %s AND %s AND INSTR(`title_cn`, ' ') = 0")
...
def main():
    ...
    with open('data/kb2vec/{}.v2.pickle'.format(ENTITY_CATEGORY), 'wb') as f:
